...
from app.models import AiSop, AppSettings, Article, InquiryType, ModelPrice, PromptTemplate, SopVersion
...
def seed():
    ...
    try:
        ...
        if db.query(PromptTemplate).count() == 0:
            ...
            tri = PromptTemplate(
                name="기본 링크 검수(판단) 프롬프트",
                purpose="triage",
                system_prompt=TRIAGE_SYSTEM,
                user_prompt_template=TRIAGE_USER,
            )
            db.add_all([gen, rev, tri])
            db.flush()
            db.add(
                AppSettings(
                    id=1,
